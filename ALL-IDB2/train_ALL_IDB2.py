...
import Model_ALL_IDB2 as Model
...
def main():
    ...
    dataset = LeukLoader(r'Path to ALL-IDB2 dataset on your computer')
    print(len(dataset))
    ...
    for fold in range (k):
        ...
        f = open(f'ALL_IDB2_T{time_window}_fold{fold}.txt', 'a')
        file = f'best_all_idb2_fold{fold}.pth'
        ...
        print(f'max accuracy for folds {max_acc} at epoch {best_epoch}')
...
def val(model, validation_loader, loss_fn, benign_class):
    ...
    for index, (images, targets) in enumerate(validation_loader):
        ...
        val_loss += loss_fn(output.squeeze(1), targets.float()).item()
